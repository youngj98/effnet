...
def main():
    ...
    if task == "weather":
        train_files = load_file_list(os.path.join(imagestes_dir, 'train.txt'))
        ...
        train_files = load_file_list(os.path.join(imagestes_dir, 'train_time_1.txt'))
    ...
    model = load_model(device, num_classes=3)
    ...
    # 수정 --> input 고려
    best_model_wts, metrics = train_model()
    ...
    # 수정 inpuit 다시 보기
    save_results(metrics, train_setting, metrics_save_dir, train_loader.dataset.labels, train_loader.dataset.predictions, val_loader.dataset.labels, val_loader.dataset.predictions, test_loader.dataset.labels, test_loader.dataset.predictions)
